  ...
  methods:{
    ...
    drawPathLine(){
      ...
      for (let i=0;i<this.lineInfo.pathline.length;i++){
        this.lineInfo.drawlineState[i] = L.polyline(this.lineInfo.pathline[i],{weight: 8, color: colorPathSet[i]}).addTo(layerGroup.)
        L.layerGroup(this.lineInfo.drawlineState[i]).addTo(map)
